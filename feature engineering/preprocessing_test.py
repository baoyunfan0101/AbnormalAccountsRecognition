...
# 读取文件
test_base = pd.read_csv('test_without_label/test_base.csv')
test_op = pd.read_csv('test_without_label/test_op.csv')
test_trans = pd.read_csv('test_without_label/test_trans.csv')
...
for col in ['sex', 'provider', 'level', 'verified', 'regist_type', 'agreement1', 'agreement2', 'agreement3',
    ...
    test_base[col] = test_base[col].apply(lambda x: x if pd.isnull(x) else x.replace('category ', ''))
...
for col in ['balance', 'balance_avg', 'balance1', 'balance1_avg', 'balance2', 'balance2_avg', 'product1_amount',
    ...
    test_base[col] = test_base[col].apply(lambda x: x if pd.isnull(x) else x.replace('level ', ''))

test_op['tm_diff'] = test_op['tm_diff'].apply(lambda x: pd.Timedelta(x, format='%d days %H:%M:%S').total_seconds())

test_trans['tm_diff'] = test_trans['tm_diff'].apply(
    lambda x: pd.Timedelta(x, format='%d days %H:%M:%S').total_seconds())

print('属性数值化完成')

# 样本编号user
X['user'] = test_base['user'].drop_duplicates()
...
for col in ['sex', 'provider', 'level', 'verified', 'regist_type', 'agreement1', 'agreement2', 'agreement3',
    ...
    X[col] = test_base[test_base['user'] == X['user']][col]
...
for idx, row in X.iterrows():
    tmp = test_op[test_op['user'] == row['user']]
...
trans_min_interval = []
for idx, row in X.iterrows():
    tmp = test_trans[test_trans['user'] == row['user']]
...
# 写入文件
X.to_csv('preprocessing_test.csv', index=None)
