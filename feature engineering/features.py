...
if __name__ == '__main__':
    ...
    label = pd.read_csv('train_with_label/train_label.csv')
    df['label'] = label['label']

    # 计算iv
    print('iv:')
    cols = df.columns.values.tolist()   # 移除'label'
    cols.pop()
    for i in cols:
        chi2_result, var_name = cal_chi2(df, i, 'label')
        interval = 5
    ...
    # 计算相关系数
    correlation = df.drop('label', axis=1).corr(method='spearman')
    print('correlation:')
    print(correlation)
